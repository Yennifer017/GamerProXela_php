...
if (isset($_GET['id']) && $worker != null && $worker->getRol() == Worker::SALESPERSON_ROL) {
    try {
        $productId = $_GET['id'];
        $productId = (int) $productId;
        $productDB = new ProductDB();
        $idSucursal = (int) $worker->getIdSucursal();
        $product = new OnSale($productId);
        $product->setIdSucursal($idSucursal);
        $product = $productDB->getOnSaleProduct(
            $product, SalespersonConnDB::getInstance()->getConnection());
